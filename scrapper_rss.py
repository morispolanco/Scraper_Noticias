...
class Scrapper:
    ...
    def recorre_diarios(self):
        ...
        for diario in diarios:
            try:
                print(f"Obteniendo noticias de {diarios[diario]['diario']}, sección {diarios[diario]['seccion']}")
                st.session_state['muestra_progreso'] = f"{diarios[diario]['diario']} --> sección {diarios[diario]['seccion']}"
    ...
    def formateo_noticias(self):
        ...
        dataframe_noticias.drop_duplicates(subset=['titulo'], inplace=True)
    ...
    def sentimientos(self, dataframe_noticias):
        with st.spinner('Descargando sentimientos para las noticias..'):  
            ...
            noticias.to_csv(f"diarios/noticias_con_sentimientos_{dia_str}.csv", index=False)
            pass
    ...
    def apila_diarios_historicos(self):
        ...
        apilados.drop_duplicates(subset=['titulo'], inplace=True)
    ...
    def genera_wordcloud(self, palabras_para_wordcloud):
        palabras_ignoradas = set(['a', 'ante', 'con', 'contra', 'de', 'desde', 'durante', 'en', 'para', 'por', 'segun', 'sin', 'sobre', 'el', 'la', 'los', 'las', '...', 'y', 'hoy', 'este', 'cuanto', 'un', 'del', 'las', 'que', 'con', 'todos', 'es', '¿qué', 'como', 'cada', 'jueves', '¿cuanto', 'hoy', 'al', 'cual', 'se', 'su', 'sus', 'lo', 'una', 'un', 'tiene', 'le', 'habia'])

        wordcloud = WordCloud(width=1920, height=1080, stopwords=palabras_ignoradas).generate(palabras_para_wordcloud)
    ...
    def run(self):
        ...
        palabras_para_wordcloud = self.transforma_letras_para_wordcloud(dataframe_noticias)
